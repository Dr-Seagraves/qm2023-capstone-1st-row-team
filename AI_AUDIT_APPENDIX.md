...
## Per Task (Individual Prompts)

### 1. Model A Fixed-Effects Setup

- **Task:** Build a two-way fixed-effects panel model for housing-price growth with clustered SEs.
- **Prompt:** "Create a PanelOLS model with entity and time fixed effects for Metro-level monthly data, clustered standard errors by entity, and save regression outputs to CSV."
- **AI Output:** Generated model scaffolding and estimation flow in `code/capstone_models.py`.
- **Verification:** We ran the script end-to-end and checked that coefficients, SEs, p-values, and sample sizes were exported correctly.
- **Critique:**
  - **What AI did well:** Fast setup of reproducible FE estimation workflow.
  - **What needed human judgment:** Variable engineering and interpretation choices.
  - **Our corrections/actions:** We adjusted final regressors and checked absorbed-effect warnings.
...
- **Critique:**
  - **What AI did well:** Implemented required tests with correct package functions.
  - **What needed human judgment:** Interpreting implications for standard errors and model caveats.
  - **Our corrections/actions:** We used clustered SEs in the final reported model.
...
- **Critique:**
  - **What AI did well:** Structured checks consistently and exported summary table.
  - **What needed human judgment:** Selecting meaningful lag windows and subgroup definitions.
  - **Our corrections/actions:** We kept lag 1/2/3 and high-vs-low affordability subgroup splits.

### 4. ARIMA Forecast (Option 2)

- **Task:** Forecast aggregate outcome 12 periods ahead with confidence intervals and naive baseline comparison.
- **Prompt:** "Use auto_arima to select p,d,q, run ADF stationarity test, fit statsmodels ARIMA, forecast 12 steps with 95% intervals, and compare RMSE to naive no-change baseline."
- **AI Output:** Implemented ARIMA workflow, forecast table, and forecast figure.
- **Verification:** We validated generated files and corrected runtime compatibility with current pandas frequency aliases.
- **Critique:**
  - **What AI did well:** Complete forecast pipeline and baseline comparison.
  - **What needed human judgment:** Interpreting why ARIMA underperformed naive in this sample.
  - **Our corrections/actions:** Updated frequency handling and documented forecasting limitations.
...
- **Critique:**
  - **What AI did well:** Fast implementation of comparable test metrics.
  - **What needed human judgment:** Deciding whether small RMSE gains justify reduced interpretability.
  - **Our corrections/actions:** Emphasized interpretability trade-offs in the memo.

### 6. Interpretation Memo Drafting

- **Task:** Draft `M3_interpretation.md` with required sections and economic interpretation.
- **Prompt:** "Draft an interpretation memo with Model A headline, economic channels, Model B summary, diagnostics implications, robustness summary, and caveats. Use values from output tables."
- **AI Output:** Produced a complete memo draft with all required headings.
- **Verification:** We cross-checked every reported statistic against exported M3 tables.
- **Critique:**
  - **What AI did well:** Organized structure and complete first draft quickly.
  - **What needed human judgment:** Causal language discipline and caveat framing.
  - **Our corrections/actions:** Tightened wording around identification limits and external validity.

## Summary (Milestone 3)

**Total AI use:** AI tools were used for model code scaffolding, diagnostics/robustness implementation, forecast/comparison workflows, and memo drafting.
