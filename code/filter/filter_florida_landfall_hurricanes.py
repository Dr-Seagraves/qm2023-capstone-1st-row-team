...
Reads: data/processed/hurricane_economic_impacts_1980_2024.csv
Writes: data/processed/hurricane_economic_impacts_1980_2024_florida_landfall.csv
...
logger = setup_logger("filter.florida_landfall")
...
OUTPUT_CSV = PROCESSED_DATA_DIR / "hurricane_economic_impacts_1980_2024_florida_landfall.csv"
...
def latest_hurdat_url() -> str:
    ...
    return f"{HURDAT_INDEX_URL}{candidates[0][1]}"
...
def point_in_polygon(x: float, y: float, rings: list) -> bool:
    if not rings:
        return False
...
def florida_geometry() -> dict:
    ...
    raise RuntimeError("Florida geometry not found in GeoJSON.")


def hurdat_florida_landfalls() -> set[Tuple[str, int]]:
    hurdat_url = latest_hurdat_url()
    logger.info("Using HURDAT2 file: %s", hurdat_url)
    ...
    landfall_names: set[Tuple[str, int]] = set()
    ...
    while idx < len(lines):
        ...
        for _ in range(entries):
            ...
            idx += 1
    logger.info("Found %d Florida landfall storm/year combinations", len(landfall_names))
...
def main() -> None:
    ...
    logger.info("Wrote %d Florida-landfall rows to %s", len(rows), OUTPUT_CSV)
